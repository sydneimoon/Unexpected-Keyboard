  ...
  implements View.OnTouchListener, Pointers.IPointerEventHandler
{
      ...
      float clipb, Paint paint, Theme.Computed.Key tc)
  {
    ...
    private int labelColor(KeyValue k, boolean isKeyDown, boolean sublabel, boolean isMainLabel) {
        ...
        if (k.hasFlagsAny(KeyValue.FLAG_KEY_FONT) || "Ctrl".equals(label) || "Fn".equals(label)) {
            if (isKeyDown) {
                int c = getPressedOrLockedColor(k);
                if (c != -1) return c;
            }
            return isMainLabel ? 0xffc0e6ad : 0xff454d41;
        }
